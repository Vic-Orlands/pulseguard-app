...
export default function Homepage() {
  ...
  return <div className="pg-page min-h-screen overflow-hidden">
    ...
    <footer className="relative isolate overflow-hidden bg-black text-white"><div aria-hidden="true" className="pointer-events-none absolute inset-x-0 top-7 z-0 select-none overflow-hidden text-center text-[clamp(4.5rem,14vw,15rem)] font-medium leading-none tracking-[-.1em] text-white/[0.07]">PULSEGUARD</div><div className="relative z-10 mx-auto grid min-h-[410px] max-w-[1440px] items-end gap-10 border-t border-[#4a4a46] px-6 pb-10 pt-36 sm:px-12 sm:pt-40 lg:grid-cols-[1.4fr_1fr_1fr]"><div><div className="scale-[.84] origin-left invert"><PulseGuardLogo /></div><p className="mt-5 max-w-xs text-sm leading-6 text-neutral-500">Observability for the parts of your product that cannot be left to guesswork.</p></div><div className="grid grid-cols-2 gap-8 text-sm"><div className="space-y-4 text-neutral-400"><p className="text-xs text-neutral-600">Product</p><a href="#product">Signals</a><a href="#integrate" className="block">Instrumentation</a><a href="#signals" className="block">Live console</a></div><div className="space-y-4 text-neutral-400"><p className="text-xs text-neutral-600">Company</p><a href="#integrate" className="block">Docs</a><a href="https://github.com" target="_blank" rel="noreferrer" className="block">GitHub</a></div></div><p className="text-xs text-neutral-600">© {new Date().getFullYear()} PulseGuard</p></div></footer>
  </div>;
}
