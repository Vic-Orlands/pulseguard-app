  ...
  return <div className="pg-page min-h-screen overflow-hidden">
    <header className="border-b border-[#e4e4df] bg-[#f7f7f5]/90 backdrop-blur-md">
      <div className="pg-shell flex h-[78px] items-center justify-between px-5 sm:px-10">
        ...
        <nav className="hidden gap-8 text-sm text-[#454540] md:flex"><a href="#product">Product</a><a href="#integrate">Docs</a><a href="#signals">Signals</a></nav>
        <div className="flex items-center gap-3"><button className="hidden text-sm sm:block" onClick={() => user ? router.push("/projects") : authenticate("login")}>{user ? "Dashboard" : "Log in"}</button>{mounted && <button className="grid size-9 place-items-center border border-[#dfdfda]" aria-label="Toggle theme" onClick={() => setTheme(theme === "dark" ? "light" : "dark")}>{theme === "dark" ? <Sun size={14} /> : <Moon size={14} />}</button>}<button className="bg-[#171716] px-4 py-2.5 text-xs font-semibold text-white hover:bg-[#ff5a1f]" onClick={() => authenticate("signup")}>Start monitoring</button></div>
      </div>
    </header>
    ...
    <main className="pg-grid">
      <section className="pg-shell relative isolate flex min-h-[800px] items-center justify-center overflow-hidden border-b border-[#e4e4df] px-5 pb-24 pt-14 text-center sm:min-h-[860px]">
        ...
        <div className="relative z-10 mx-auto flex max-w-4xl flex-col items-center">
          <h1 className="max-w-4xl text-[clamp(3.2rem,6.2vw,6.8rem)] font-semibold leading-[.93] tracking-[-.075em]">Know what changed <span className="pg-signal">before your users do.</span></h1>
          <p className="mt-7 max-w-xl text-base leading-7 text-[#73736e] sm:text-lg">Real-time observability for modern systems. Detect issues, own incidents, and ship with confidence.</p>
          <div className="mt-8 flex items-center gap-6"><button className="pg-action pg-action-primary" onClick={() => authenticate("signup")}>Start monitoring</button><a className="flex items-center gap-2 text-sm font-medium" href="#integrate">Explore the docs <ArrowRight size={15} /></a></div>
          <div className="mt-20 w-full max-w-3xl border border-[#e1e1dc] bg-white p-2 shadow-[0_22px_50px_rgba(30,30,20,.09)] sm:p-3">
            ...
            <div className="flex items-center gap-3 border border-[#e3e3de] px-4 py-3 text-left"><Search size={16} className="text-[#8a8a85]" /><span className="flex-1 text-sm text-[#4b4b47]">service:checkout</span><button className="grid size-9 place-items-center bg-[#ff5a1f] text-white"><ArrowRight size={16} /></button></div>
          </div>
        </div>
      </section>
    ...
    <footer className="bg-black text-white"><div className="pg-shell grid min-h-[430px] items-end gap-12 border-[#252525] px-6 py-10 sm:px-12 lg:grid-cols-[1.4fr_1fr_1fr]"><div><div className="scale-[.84] origin-left invert"><PulseGuardLogo /></div><p className="mt-7 max-w-xs text-sm leading-6 text-neutral-500">Observability for the parts of your product that cannot be left to guesswork.</p></div><div className="grid grid-cols-2 gap-8 text-sm"><div className="space-y-4 text-neutral-400"><p className="text-xs text-neutral-600">Product</p><a href="#product">Signals</a><a href="#integrate" className="block">Instrumentation</a><a href="#signals" className="block">Live console</a></div><div className="space-y-4 text-neutral-400"><p className="text-xs text-neutral-600">Company</p><a href="#integrate" className="block">Docs</a><a href="https://github.com" target="_blank" rel="noreferrer" className="block">GitHub</a></div></div><p className="text-xs text-neutral-600">© {new Date().getFullYear()} PulseGuard</p></div></footer>
  </div>;
}
