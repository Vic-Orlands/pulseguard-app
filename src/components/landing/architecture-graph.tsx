...
export function ArchitectureGraph() {
  ...
  return <div className="grid items-center gap-12 text-left lg:grid-cols-[1.15fr_.85fr]"><div><p className="pg-label">Pipeline architecture</p><h3 className="mt-4 text-3xl font-medium tracking-[-.05em] text-[#272725] dark:text-white">Follow every signal through the stack.</h3><p className="mt-4 max-w-xl text-sm font-light leading-6 text-[#73736e] dark:text-[#a3a3a3]">Select a node to see how telemetry moves from your application to the place where your team investigates it.</p><div className="mt-10 rounded-2xl border border-[#dfdfda] bg-transparent p-6 dark:border-[#3b3b3b] dark:bg-[#121212]"><div className="flex justify-center"><NodeButton nodeKey="app" active={activeKey === "app"} onClick={() => setActiveKey("app")} /></div><ArrowDown className="mx-auto my-3 text-[#b1b1ac] dark:text-[#4a4a4a]" size={16} /><div className="flex justify-center"><NodeButton nodeKey="otel" active={activeKey === "otel"} onClick={() => setActiveKey("otel")} /></div><div className="mx-auto my-5 h-7 w-[72%] border-x border-t border-[#d4d4cf] dark:border-[#3b3b3b]" /><div className="grid grid-cols-3 gap-3"><NodeButton nodeKey="loki" active={activeKey === "loki"} onClick={() => setActiveKey("loki")} /><NodeButton nodeKey="tempo" active={activeKey === "tempo"} onClick={() => setActiveKey("tempo")} /><NodeButton nodeKey="prometheus" active={activeKey === "prometheus"} onClick={() => setActiveKey("prometheus")} /></div><div className="mx-auto my-5 h-7 w-[72%] border-x border-b border-[#d4d4cf] dark:border-[#3b3b3b]" /><div className="flex justify-center"><NodeButton nodeKey="grafana" active={activeKey === "grafana"} onClick={() => setActiveKey("grafana")} /></div></div></div><div className="rounded-2xl border border-[#dfdfda] bg-transparent p-6 dark:border-[#3b3b3b] dark:bg-[#121212] sm:p-8"><div className="flex items-start justify-between border-b border-[#e6e6e1] pb-5 dark:border-[#303030]"><div><p className="pg-label">Selected node</p><h4 className="mt-2 text-xl font-medium text-[#272725] dark:text-white">{activeNode.name}</h4></div><span className="grid size-10 place-items-center rounded-lg border border-[#dfdfda] text-[#ff5a1f] dark:border-[#3b3b3b]"><ActiveIcon size={18} /></span></div><ul className="mt-6 space-y-4">{activeNode.details.map((detail) => <li key={detail} className="flex gap-3 text-sm font-light leading-6 text-[#73736e] dark:text-[#a3a3a3]"><span className="mt-2 size-1.5 shrink-0 rounded-full bg-[#ff5a1f]" />{detail}</li>)}</ul><div className="mt-8 border-t border-[#e6e6e1] pt-4 font-mono text-[10px] uppercase tracking-wider text-[#777772] dark:border-[#303030] dark:text-[#a3a3a3]">Node id · {activeKey.toUpperCase()}</div></div></div>;
}
