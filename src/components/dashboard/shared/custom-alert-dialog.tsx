...
export function CustomAlertDialog({
  ...
  return (
    <AlertDialog>
      ...
      <AlertDialogContent className="bg-[var(--panel-bg-0a)] border border-zinc-800 shadow-2xl p-6 !max-w-sm rounded-xl text-white">
        <AlertDialogHeader className="mb-5 text-left flex flex-col gap-1.5">
          ...
          <AlertDialogDescription className="text-xs text-zinc-500 dark:text-zinc-400 font-sans leading-relaxed">
            {description}
          </AlertDialogDescription>
        </AlertDialogHeader>
        <AlertDialogFooter className="flex space-x-2.5 mt-2">
          <AlertDialogCancel className="flex-1 border border-zinc-800 hover:border-zinc-700 bg-transparent hover:bg-zinc-100 dark:hover:bg-zinc-900/60 text-zinc-500 dark:text-zinc-400 hover:text-zinc-900 dark:hover:text-white text-xs h-9 px-4 rounded-[5px] shadow-none font-semibold cursor-pointer transition-all duration-200">
            Cancel
          </AlertDialogCancel>
          <AlertDialogAction
            onClick={onConfirm}
