...
// Metrics holds custom Prometheus metrics
type Metrics struct {
...
// InitMetrics initializes all Prometheus-compatible metrics
func InitMetrics() (*Metrics, error) {
    meter := otel.Meter("pulseguard")
    ...
    httpErrorsTotal, err := meter.Int64Counter(
        "http_errors_total",
        metric.WithDescription("Total count of HTTP error responses (4xx and 5xx)"),
    )
    if err != nil {
        return nil, err
    }
    ...
    appErrorsTotal, err := meter.Int64Counter(
        "app_errors_total",
        metric.WithDescription("Total count of application errors"),
    )
    if err != nil {
        return nil, err
    }
