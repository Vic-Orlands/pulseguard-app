...
func (r *SessionRepository) CreateSession(ctx context.Context, session *models.Session) error {
    query := `
        INSERT INTO sessions (session_id, project_id, user_id, start_time, error_count, event_count, pageview_count, created_at)
        VALUES ($1, $2, $3, $4, $5, $6, $7, $8)
        ON CONFLICT (session_id) DO NOTHING
    `
    _, err := r.db.ExecContext(ctx, query, session.SessionID, session.ProjectID, session.UserID, session.StartTime, session.ErrorCount, session.EventCount, session.PageviewCount, session.CreatedAt)
    if err != nil {
...
func (r *SessionRepository) GetSessions(ctx context.Context, projectID string, start, end time.Time) ([]*models.Session, error) {
    query := `
        SELECT session_id, project_id, user_id, start_time, end_time, duration_ms, error_count, event_count, pageview_count, created_at
        FROM sessions
    ...
    for rows.Next() {
        ...
        if err := rows.Scan(&s.SessionID, &s.ProjectID, &s.UserID, &s.StartTime, &endTime, &durationMs, &s.ErrorCount, &s.EventCount, &s.PageviewCount, &s.CreatedAt); err != nil {
